...
class AlignRestore(object):
    ...
    def restore_img(self, input_img, face, affine_matrix):
        ...
        # Heritage Fix: Pre-calibration before blending
        inv_face = (inv_face / 2 + 0.5).clamp(0, 1) * 255

        input_img_tensor = rearrange(torch.from_numpy(input_img).to(device=self.device, dtype=self.dtype), "h w c -> c h w")
        inv_mask = kornia.geometry.transform.warp_affine(
            self.mask, inv_affine_matrix, (h, w), padding_mode="zeros"
        )  # (1, 1, h_up, w_up)

        # Heritage Fix: Adaptive Blending Mask
        inv_mask_erosion = kornia.morphology.erosion(
            inv_mask,
            torch.ones(
                (int(4 * self.upscale_factor), int(4 * self.upscale_factor)), device=self.device, dtype=self.dtype
            ),
        )

        inv_mask_erosion_t = inv_mask_erosion.squeeze(0).expand_as(inv_face)
        
        # V27.4 Heritage Color Transfer (Fixes "Transparent Square" lighting mismatch)
        # We match the mean color of the generated face to the target skin area
        with torch.no_grad():
            mask_bool = inv_mask_erosion_t > 0.5
            if torch.any(mask_bool):
                target_mean = torch.mean(input_img_tensor[mask_bool].float())
                source_mean = torch.mean(inv_face[mask_bool].float())
                color_shift = (target_mean - source_mean) * 0.8 # Conservative shift
                inv_face = (inv_face + color_shift).clamp(0, 255)

        pasted_face = inv_mask_erosion_t * inv_face
        total_face_area = torch.sum(inv_mask_erosion.float())
        
        # Heritage Softness: Increased blur radius for invisible seams
        w_edge = int(total_face_area**0.5) // 12
        erosion_radius = w_edge * 2

        # Run on CPU for stability in large frames
        inv_mask_erosion_np = inv_mask_erosion.squeeze().cpu().numpy().astype(np.float32)
        inv_mask_center = cv2.erode(inv_mask_erosion_np, np.ones((erosion_radius, erosion_radius), np.uint8))
