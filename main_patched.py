...
app = FastAPI(title="Brasil AI Industrial Avatar Engine - V13.0 HD")

# Configurações de Caminhos (Volume Persistente)
...
class DIDClipRequest(BaseModel):
    ...
    batch_size: Optional[int] = 8 # Adicionado para compatibilidade com o Orquestrador

@app.get("/health")
def health():
    return {"status": "ok", "gpu": os.path.exists("/dev/nvidia0"), "vram_warning": False}
...
def run_inference(job_id, audio_url, template, resolution):
    try:
        ...
        # 4. LatentSync Inference (Strict HD V13.0)
        print(f"🚀 [WORKER] Iniciando Job {job_id} em {resolution}px...")
        env = os.environ.copy()
        env["PYTHONPATH"] = "."
        ...
        # Nota: LatentSync inference.py tipicamente herda a resolução do template_video.
        # Nossos templates em /app/data/templates já são 512px.
        
        inf_proc = subprocess.run(cmd, env=env, capture_output=True, text=True, cwd="/app")
        ...
        if mux_proc.returncode == 0:
            ...
            jobs_db[job_id]["result_url"] = f"/app/temp/{job_id}_final.mp4"
            print(f"✅ [WORKER] Job {job_id} concluído em HD!")
        else:
            print(f"⚠️ [WORKER] Muxing falhou: {mux_proc.stderr}")
            jobs_db[job_id]["status"] = "completed_no_audio"
            jobs_db[job_id]["result_url"] = f"/app/temp/{job_id}_video.mp4"
